    ...
    if (fin.is_open() && fpr.is_open()) {
      ...
      hls::stream<result_t> layer13_out("layer13_out");
            ...
            if (e % CHECKPOINT == 0) {
                ...
                for(int i = 0; i < N_FILT_13*N_OUTPUTS_13; i++) {
                  std::cout << pr[i] << " ";
                }
                std::cout << std::endl;
                std::cout << "Quantized predictions" << std::endl;
                // hls-fpga-machine-learning insert quantized
                nnet::print_result<result_t, N_FILT_13*N_OUTPUTS_13>(layer13_out, std::cout, true);
            }
            e++;

            // hls-fpga-machine-learning insert tb-output
            nnet::print_result<result_t, N_FILT_13*N_OUTPUTS_13>(layer13_out, fout);
        }
        fin.close();
        fpr.close();
    } else {
